...
def handle_jenkins_server_webhooks(request):
    ...
    command = """curl -X POST http://jenkins_api:119b7414e30b8d610b40e3574ccf32a829@localhost:8080/job/server/build/\?token\=my-token --data-urlencode json='{"parameter": [{"name":"BRANCH_NAME", "value":"%s"}, {"name":"CONTAINS_MIGRATION", "value":"%s"}]}' "" % (branch_name, contains_migration)
